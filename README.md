...
![The same comparison on a real recording](docs/notch_comparison_real.png)

The third figure repeats the measurement on a real EEG--fMRI recording, sub-0011 run 1, 63
channels over 493 s, and is generated by
[`docs/make_notch_figure_real.py`](docs/make_notch_figure_real.py). It corroborates the
synthetic result rather than replacing it: the data is not public, so unlike the two figures
above this one cannot be regenerated from the repository alone.

Both arms are aimed at the identical 57 lines the plan authorised, so what is compared is how
each transform takes them. Fitting them costs 9.0 Hz of the 97 Hz analysed band and notching
them costs 49.0 Hz -- against 8.9 Hz and 50.0 Hz on the generated dataset, which is closer
agreement than the synthetic figure alone would justify claiming. Target prominence falls from
8.5 dB over background to 0.2 dB median under the fit, and to --39.0 dB under the notch.

Two things appear here that generated data does not produce. First, a real comb is not evenly
spaced: 16 of the 56 gaps in this target list fall below 1.19 Hz and the smallest is 0.439 Hz,
...
generated one therefore agree to 0.3 Hz on the fit and 1.7 Hz on the notch, which is a firmer
claim than the synthetic figure alone supports. The figure stays on run 1 because the baseline
is the harder recording in the one way that shows on the page: its comb is tighter still, 26
of 62 gaps below 1.19 Hz and the smallest 0.109 Hz, so nine targets merge instead of five and
one surviving line falls inside the magnified window.

Those survivors are worth naming, and they are not the drift. In both recordings the single
worst target belongs to a pair spaced under 0.5 Hz -- 5.2 dB on run 1, 7.8 dB on the baseline.
The baseline is the better evidence of the two: it has ten such targets, they start no
stronger than the rest at 7.2 dB over background against the others' 8.5 dB, and they still
end higher, at 1.8 dB against 0.4 dB. Run 1's two start at 14.4 dB, well above the rest, so on
their own they would not separate close spacing from simply being large. The mechanism is not
established here, and it is not a plain resolution limit, since 0.5 Hz sits well above the
27 mHz the estimation window resolves. What the measurement does show is that tight spacing is
where both stages give something up, and that they give up different things: the fit leaves
those lines a decibel or two high and keeps the band, the notch takes them and pays 50 Hz of
band.
