...
the 2 dB threshold once, and writes an evidence-bounded BIDS derivative that declares the
bandwidth both stages destroyed. ``verify`` re-derives each stage from the source and
reproduces the derivative sample for sample, and ``psd`` visualises the result.
